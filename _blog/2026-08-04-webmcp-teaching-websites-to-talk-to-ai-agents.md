...
There's also the question of *what decides which tool to call*. Today that's usually a slow, expensive LLM call per step. But a WebMCP page hands you a bounded, typed set of tools, which is exactly the shape a fast "decision" model wants. I wired [Jev](/blog/2026-09-20-jev-the-ai-that-returns-a-decision-not-a-paragraph/) (Typesafe's System One model) up to this demo's 13 tools as the router: it picks the right tool in about 100ms with a calibrated confidence, and gates the one consequential tool on an honest number instead of an LLM's guessed one. It's a live side-by-side (Jev vs an LLM) at [the routing demo](https://jev-live-demo-production.up.railway.app/webmcp). The division of labor is clean: WebMCP defines what can be done, a fast calibrated model routes and gates, and you spend the big LLM only on the genuinely open-ended parts.
